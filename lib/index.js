...
exports.init = function (conf) {
	...
	var models = {};
	var modelIgnore = conf.modelIgnorePattern ? new RegExp(conf.modelIgnorePattern) : null
	log('create model registry')
	log('from file path:', conf.modelPath)
	if (modelIgnore) {
		log('ignore files matching:', modelIgnore)
	} else {
	...
	readDir(conf.modelPath).forEach(function (file) {
		...
		if (modelIgnore && modelIgnore.test(file)) {
			log('-- ignored', file)
			return
		}
